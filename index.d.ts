...
// Why? Because I wasn't able to make the hardcoded object conform to the shape of the expect object by Table.query()

// I tried copying in the values one by one and TS 'infinite depth' errors kept cropping updated_at

// TODO: Rewrite these hacky type definitions to reflect the actual shape of tables. These types do exist in /src/models/ but they're like weird Knex extensions of the database tables that can only be interfaced via GraphQL and Knex queries in order to avoid TS errors.
export interface IStepDB {
  id: string;
  name?: string;
  flowId: string;
  key?: string;
